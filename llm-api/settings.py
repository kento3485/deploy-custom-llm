...
class Settings(BaseSettings):
    ...
    ELASTICSEARCH_URL: str = Field(
        ...
        description="Elasticsearch URL (use http://elasticsearch:9200 on Render)",
    )
    ELASTIC_USER: str = Field(
        default="",
        env="ELASTIC_USER",
        description="Elasticsearch username (empty to disable auth)",
    )
    ELASTIC_PASSWORD: str = Field(
        default="",
        env="ELASTIC_PASSWORD",
        description="Elasticsearch password (empty to disable auth)",
    )
